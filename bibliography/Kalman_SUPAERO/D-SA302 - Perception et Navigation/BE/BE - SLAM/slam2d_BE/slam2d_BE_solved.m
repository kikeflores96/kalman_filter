...
% ----------------------------------------------------------------
%
%  HELP NOTES: PLEASE SEE THE COURSE SLIDES FOR NICER NOTES !!!
%
% ----------------------------------------------------------------
%
%
%  HELP NOTES: Please refer to the following sketch and to the notes below
%
%
%     Y axis
%
%     |
%     |      
%  y1 +         *               <-- landmark L1 = (x1, y1)   = x(l1)
%     |
%     |
%     |
%  y2 +                   *     <-- landmark L2 = (x2, y2)   = x(l2)
%     |    __
%  yr +    \/                   <-- robot:      (xr,yr,ar)   = x(r)
%     |
%     |
%     +----+----+---------+-------------  X axis
%          xr   x1        x2
...
%           further below.
%       rl: pointers to robot and one landmark. Build it from r and l, 
%           as rl = [r,l]
%       rm: pointers to robot and all landmarks (the currently used map).
%           Build it using r and m vectors with rm = [r,m]
%
%     Therefore (see the sketch above) we have:
%                 x(r)     estimated robot state, pointed by 'r'
%                 x(l)     estimated state of one landmark, pointed by 'l'
%                 P(r,r)   covariance of the robot
%                 P(l,l)   covariance of one landmark
%                 P(r,l)   cross-variance between robot and a lmk
%                 P(rm,rm) current full covariance -- the rest is
%                 unused.
%
...
%                                              to landmark i
%       * lm_lmk_pointer(i)                 -> recover landmark pointer
...
P(r,r) = diag([0.0 0.0 0.0]);    % initialize robot covariance
...
for t = 1:200
    ...
    for i = lids
        ...
        Z = E*[P(r,r) P(r,l);P(l,r) P(l,l)]*E'+S;
