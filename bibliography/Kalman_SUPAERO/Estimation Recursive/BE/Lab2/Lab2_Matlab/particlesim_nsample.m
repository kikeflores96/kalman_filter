...
listNumSamples = [50 100 200 500];
listEQM = zeros(length(listNumSamples),N);
for tsim=1:numSim
...
for tSamples =1:length(listNumSamples)
    
    numSamples = listNumSamples(tSamples);
%% section 4
...
for t=1:N-1
    ...
    X0=mean(xu,2);
    EQM = (X0-x(t,:)')'*(X0-x(t,:)');
    listEQM(tSamples,t)= listEQM(tSamples,t) + sqrt(EQM);
end
end
end
...
for tSamples =1:length(listNumSamples)
    numSamples = listNumSamples(tSamples);
    plot(1:N-1,listEQM(tSamples,1:N-1),'LineWidth',6)
end
