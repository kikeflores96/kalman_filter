...
close all;
global RA PA
YUMAdata;         % 26 March 2014 almanac data with 31 satellites
[rows,cols] = size(Yuma);
NoSats  = rows;   % Number Of Satellites
...
for t=0:NoSecs,
    k = k + 1;
    Time(k) = t;
    HGPS = [HSatSim(t,Lat,Lon,Alt),zeros(NoSats,6),eye(NoSats)];
    NoSatsAvail = NoSats;
    %
...
subplot(2,1,1),
plot(Time(10:Nsecs)/3600,RMSDiff,'k-','LineWidth',1.8);
set(gca,'FontSize',14);
text(2,.8*max(RMSDiff),['{\sigma}_{Kalman} = ',num2str(round(10*RMSKal)/10),' [m]'],'FontSize',14,'HorizontalAlignment','center');
title('DIFFERENCE BETWEEN SCHMIDT-KALMAN AND KALMAN (RMS ERR/AXIS)','FontSize',14);
xlabel('TIME [hr]','FontSize',14);
ylabel('DIFFERENCE [m]','FontSize',14);
subplot(2,1,2),
plot(9/3600,0,'w.',NoSecs/3600,12,'w.',Time(10:Nsecs)/3600,NoSatsInView(10:Nsecs),'k-','LineWidth',1.8);
set(gca,'FontSize',14);
%title([num2str(NoSecs/60/60),' Hrs of Simulated GPS Operation']);
xlabel('TIME [hr]','FontSize',14);
ylabel('# SATELLITES USED','FontSize',14);
%%  
