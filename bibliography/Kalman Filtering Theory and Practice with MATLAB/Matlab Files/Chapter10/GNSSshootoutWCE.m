% GNSSshootoutWCE (with clock errors)
%
% Solution to Problem 10.8 in
...
% at 90 kph average speed and a nominal low-cost clock with relative
% stability of 1e-9 part/part over 1 second.
%
...
%
% Name  Min  Max  Source
% MODL3  17   19  Model No. 3 in Table 10.2
% MODL5  17   19  Model No. 5 in Table 10.2
% MODL6  20   22  Model No. 6 in Table 10.2
% FIG8   13   15  2D model using only along-track position and velocity.
%
% Uses GPS configuration loaded as Yuma ephemeris data.
%
clear all;
close all;
%
...
%
% Assign parameters for clock model
%
Phiclock    = [1, Dt; 0, 1];
Qclock      = [ 0.06 , 0.05 ; 0.05 , 0.10 ];
Cclock      = chol(Qclock);
x0clock     = Cclock*randn(2,1);
Hclock      = [ -1 , 0 ];
P0clock     = [1, 0; 0, 0.01];
%
% Assign parameters for GNSS pseudorange model.
%
...
%
% Statistical parameters of pseudorange error due to propagation delays
%
...
%
% Pseudorange measurement noise covariance
%
...
Phi1E   = [1 Dt; 0 1];
Phi1D   = [1 Dt; 0 1];
Phi1    = [Phi1N,zeros(2,NoSats+6);zeros(2),Phi1E,zeros(2,NoSats+4);
     zeros(2,4),Phi1D,zeros(2,NoSats+2);
     zeros(2,6),Phiclock,zeros(2,NoSats);zeros(NoSats,8),PhiPR];
Q1N     = [0,0;0,MSdVN];
Q1E     = [0,0;0,MSdVE];
Q1D     = [0,0;0,MSdVD];
Q1      = [Q1N,zeros(2,NoSats+6);
    zeros(2),Q1E,zeros(2,NoSats+4);
    zeros(2,4),Q1D,zeros(2,NoSats+2);zeros(2,6),Qclock,zeros(2,NoSats);
    zeros(NoSats,8),QPR];
P1N     = [RMSPosN^2,0;0,RMSVelN^2];
P1E     = [RMSPosE^2,0;0,RMSVelE^2];
P1D     = [RMSPosD^2,0;0,RMSVelD^2];
P1      = [P1N,zeros(2,NoSats+6);
           zeros(2),P1E,zeros(2,NoSats+4);
           zeros(2,4),P1D,zeros(2,NoSats+2);
           zeros(2,6),P0clock,zeros(2,NoSats);
           zeros(NoSats,8),P0PR];
%
% Initialize satellite signal delay error estimates at true values
%
x1      = [zeros(6,1);x0clock;xPR];
x2      = [zeros(6,1);x0clock;xPR];
x3      = [zeros(9,1);x0clock;xPR];
x4      = [zeros(2,1);x0clock;xPR];
%
...
Phi2    = [Phi2N,zeros(2,NoSats+6);
    zeros(2),Phi2E,zeros(2,NoSats+4);
     zeros(2,4),Phi2D,zeros(2,NoSats+2);
     zeros(2,6),Phiclock,zeros(2,NoSats);
     zeros(NoSats,8),PhiPR];
Q2      = [Q2N,zeros(2,NoSats+6);
    zeros(2),Q2E,zeros(2,NoSats+4);
    zeros(2,4),Q2D,zeros(2,NoSats+2);
    zeros(2,6),Qclock,zeros(2,NoSats);
    zeros(NoSats,8),QPR];
P2      = [P2N,zeros(2,NoSats+6);
    zeros(2),P2E,zeros(2,NoSats+4);
    zeros(2,4),P2D,zeros(2,NoSats+2);
    zeros(2,6),P0clock,zeros(2,NoSats);
    zeros(NoSats,8),P0PR];
%
...
Phi3    = [Phi3N,zeros(3,NoSats+8);
           zeros(3),Phi3E,zeros(3,NoSats+5);
           zeros(3,6),Phi3D,zeros(3,NoSats+2);
           zeros(2,9),Phiclock,zeros(2,NoSats);
           zeros(NoSats,11),PhiPR];
Q3      = [Q3N,zeros(3,NoSats+8);
    zeros(3),Q3E,zeros(3,NoSats+5 );
    zeros(3,6),Q3D,zeros(3,NoSats+2);
    zeros(2,9),Qclock,zeros(2,NoSats);
    zeros(NoSats,11),QPR];
P3      = [P3N,zeros(3,NoSats+8);
    zeros(3),P3E,zeros(3,NoSats+5);
    zeros(3,6),P3D,zeros(3,NoSats+2);
    zeros(2,9),P0clock,zeros(2,NoSats);
    zeros(NoSats,11),P0PR];
%
...
Phi30  = [Phi3N0,zeros(3,NoSats+8);
          zeros(3),Phi3E0,zeros(3,NoSats+5);
          zeros(3,6),Phi3D0,zeros(3,NoSats+2);
          zeros(2,9),Phiclock,zeros(2,NoSats);
          zeros(NoSats,11),PhiPR];
Q0     = zeros(9);
...
P3E0(1,1) = 1e6;
P3D0(1,1) = 1e4;
P30    = [P3N0,zeros(3,NoSats+8);
          zeros(3),P3E0,zeros(3,NoSats+5);
          zeros(3,6),P3D0,zeros(3,NoSats+2);
          zeros(2,9),P0clock,zeros(2,NoSats);
          zeros(NoSats,11),P0PR];
%
...
Phi4        = [PhiState,zeros(2,NoSats+2);
               zeros(2),Phiclock,zeros(2,NoSats);
               zeros(NoSats,4),PhiPR];
q4          = SigmaPhiDot^2*(1-exp(-2*Dt/TauPhiDot));
Q4          = [zeros(1,NoSats+4);
               0,q4,zeros(1,NoSats+2);
               zeros(2),Qclock,zeros(2,NoSats);
               zeros(NoSats,4),QPR];
P4          = [SigmaPhi^2,zeros(1,NoSats+3);
               0,SigmaPhiDot^2,zeros(1,NoSats+2);
               zeros(2),P0clock,zeros(2,NoSats);
               zeros(NoSats,4),P0PR];
%
% RESUME HERE
%
...
for t=0:Dt:hours*3600*Dt+StartTime,
    ...
    %
    H1      = [HGPS(:,1),zeros(NoSats,1),HGPS(:,2),zeros(NoSats,1),HGPS(:,3),zeros(NoSats,1),ones(NoSats,1),zeros(NoSats,1),HPR];
    H2      = [HGPS(:,1),zeros(NoSats,1),HGPS(:,2),zeros(NoSats,1),HGPS(:,3),zeros(NoSats,1),ones(NoSats,1),zeros(NoSats,1),HPR];
    H3      = [HGPS(:,1),zeros(NoSats,2),HGPS(:,2),zeros(NoSats,2),HGPS(:,3),zeros(NoSats,2),ones(NoSats,1),zeros(NoSats,1),HPR];
    ...
    H4 = [HGPS*dPosdphi,zeros(NoSats,1),ones(NoSats,1),zeros(NoSats,1),HPR];
    %
...
disp('FIGURE-EIGHT GPS NAVIGATION TRACK SIMULATION WITH');
disp('4 DIFFERENT VEHICLE DYNAMIC MODELS AND CLOCK ERRORS');
disp(' ');
disp(['INITIAL ',num2str(StartTime),'-SECOND STATIONARY SIMULTION']);
disp('TO ALLOW SETTLING OF STAT-UP TRANSIENT:');
disp(' ');
