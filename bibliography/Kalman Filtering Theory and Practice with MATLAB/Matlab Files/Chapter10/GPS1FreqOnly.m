...
%%  
%
% PERFORMANCE ANALYSIS OF SINGLE-FREQUENCY GPS RECEIVER NAVIGATING ON A
...
% ~30 UNCOMPENSATED IONO CORRECTION ERRORS
%
close all;
clear all;
%
...
sigmaIono      = 10;  % RMS pseudorange error due to uncompensated propagation delay [m]
tauIono        = 600; % correlation time of uncompensated pseudorange errors [s]
%
% RECEIVER PSEUDORANGE MEASUREMENT NOISE
%
sigmaPRMN        = 10;  % RMS pseudorange measurement noise [m]
%
% STATE TRANSITION MATRIX FOR RESIDUAL IONO DELAY ERRORS
%
PhiIono          = exp(-Dt/tauIono)*eye(NumSats);
%
% SATELLITE SIGNAL PSEUDORANGE NOISE COVARIANCE MATRIX
%
R            = sigmaPRMN^2*eye(NumSats);
%
% PSEUDORANGE PROCESS NOISE COVARIANCE FOR UNCOMPENSATED IONO DELAY ERRORS
%
QIono            = sigmaIono^2*(1 - exp(-2*Dt/tauIono))*eye(NumSats);
stdevIono        = sigmaIono*sqrt(1 - exp(-2*Dt/tauIono)); % RMS process n.
%
% Initial covariance matrix of pseudorange error uncertainty
%
PIono0           = sigmaIono^2*eye(NumSats); % initial pseudorange uncertainty
%
...
%
% Uncompensated iono signal delay error model parameters
%
%qSat     = sigmaIono^2*(1-exp(-2*Dt/tau));
%QSat     = qSat*eye(NumSats);
%PhiSat   = exp(-Dt/tauIono)*eye(NumSats);
%PSat     = sigmaIono^2*eye(NumSats);
%
Phi     = [PhiVeh,zeros(9,NumSats+2);zeros(2,9),Phiclock,zeros(2,NumSats);zeros(NumSats,11),PhiIono];
P       = [PVeh,zeros(9,NumSats+2);zeros(2,9),P0clock,zeros(2,NumSats);zeros(NumSats,11),PIono0];
P       = .5*(P+P');
Q       = [QVeh,zeros(9,NumSats+2);zeros(2,9),Qclock*Dt,zeros(2,NumSats);zeros(NumSats,11),QIono];
Q       = .5*(Q+Q');
%
% Initialize the Monte Carlo state estimation error, using its covariance matrix
%
for k=1:NumSats+11,
    err(k) = randn(1)*sqrt(P(k,k));
end;
%
% Initial values of Monte Carlo simulation estimate
%
[xENU,vENU,aENU,aSensedENU,omegaENU,omegaSensedENU] = Big8TrackSimENU(0,LatRad,LonRad,Alt);
%
...
for t=0:Dt:seconds,
    ...
    H           = [HGNSS,zeros(NumSats,6),ones(NumSats,1),zeros(NumSats,1),-eye(NumSats)];
    ...
    RMSHor      = [RMSHor,sqrt(P(1,1)+P(2,2))];
...
figure;
plot(time(100:endoftime)/3600,RMSHor(100:endoftime),'k-','LineWidth',2);
set(gca,'FontSize',14);
title('SINGLE-FREQUENCY GPS RECEIVER','FontWeight','bold','FontSize',14);
xlabel('TIME [hr]');
...
ylabel('RMS POS. ERR. [m]','FontWeight','bold','FontSize',12);
%legend('Hor.','Vert.');
subplot(2,2,2),
