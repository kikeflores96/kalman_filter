...
%  
% GENERATES AN 11-PARAMETER ERROR BUDGET FOR A LOW-ACCURACY INERTIAL
% NAVIGATION SYSTEM WITH ABOUT 10 NAUTICAL MILES PER HOUR CEP RATE, USING 
...
%
% ERROR BUDGET CAN BE saveD TO FILE 'INSEBLoAcc.mat'
%
...
EB(4)   = 9.8e-4;
EB(5)   = 1e-4;
EB(6)   = 0.19/3600/180*pi;       % 0.19 deg/hr
EB(7)   = 1e-3;                   % 1e-3 for 10 NMi/Hr CEP rate
EB(8)   = 1;
EB(9)   = 2;
EB(10)  = 3600;
EB(11)  = 0.1;
...
title('~10 NMi/Hr ''LOW ACCURACY'' INS');
%
figure;
...
%
%
response = input('Save error budget to file ''INSEBLoAcc.mat'' ?  (Yes or No): ','s');
if (response(1)=='Y')|(response(1)=='y')
    save 'INSEBLoAcc' 'EB';
    disp('Error budget saved to file INSEBLoAcc.mat');
end;
%
