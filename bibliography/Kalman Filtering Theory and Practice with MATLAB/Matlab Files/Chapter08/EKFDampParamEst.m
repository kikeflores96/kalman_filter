% DampParamEst.m (Nonlinear estimation of damping coefficient)
%
% Example 8.4 in
% Kalman Filtering: Theory and Practice, 4th edition
...
disp('a VERTICAL mass-spring system with UNKNOWN constant damping');
disp('and known gravity forcing, plus white noise.');
disp(' ');
...
% Model parameters
%
zeta  = 0.1;     % TRUE damping factor (unitless)
...
R     = .001;     % measurement noise variance (ft^2)
omega = 10;       % undamped resonant frequency (rad/s)
...
clf;
subplot(3,1,1),plot(t,x1,'k+',tm,xh1,'kx');
%legend('True','Est.');
xlabel('Time (sec)');ylabel('Position (ft)');title('True (+) and extimated (\times) states.');
subplot(3,1,2),plot(t,x2,'k+',tm,xh2,'kx');
%legend('True','Est.');
xlabel('Time (sec)');ylabel('Velocity (fps)');
subplot(3,1,3),plot(t,x3,'k+',tm,xh3,'kx');
%legend('True','Est.');
